...
function contrastadoInfo(a) {
  if (!a.pco_person_id) {
    return { ok: false, reason: 'sin_ficha', label: 'No', guidance: 'No encontramos su ficha en Planning Center. Pregúntale si hay algún fallo (por ejemplo, un email o teléfono distinto al que usó) o si aún no se ha registrado.' };
  }
  const m = mismatches(a);
  if (m.length) {
    return { ok: false, reason: 'mismatch', label: 'No', mismatched: m.map((k) => LABEL[k]), guidance: `Dice tener ${joinEs(m.map((k) => LABEL[k]))}, pero no consta en Planning Center. Contacta con el equipo de PCO de tu campus para corregirlo.` };
  }
  return { ok: true, reason: 'ok', label: 'Sí' };
}
