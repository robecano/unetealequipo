...
const AREAS = [
  ...
  { name: 'Locales', pos: 15, image: '/img/areas/locales.jpg', icon: '🏛️', desc: 'Equipos que cuidan los espacios y la operativa de los domingos.', teams: [
  ...
  { name: 'Creativos', pos: 0, image: '/img/areas/creativos.jpg', icon: '🎨', desc: 'Equipos que comunican y expresan creativamente la visión de la iglesia.', teams: [
  ...
  { name: 'Conexiones', pos: 20, image: '/img/areas/conexiones.jpg', icon: '🤝', desc: 'Ayuda a cada persona a sentirse bienvenida y encontrar su siguiente paso.', teams: [
  ...
  { name: 'Kids', pos: 0, image: '/img/areas/kids.jpg', icon: '🧒', desc: 'Comunidad para niños de 0 a 12 años, con reuniones cada domingo.', teams: [
  ...
  { name: 'Comunidades', pos: 5, notice: ORGANIZA_NOTE, image: '/img/areas/comunidades.jpg', icon: '👥', desc: 'Espacios para conectar, pertenecer y crecer juntos entre semana.', teams: [
  ...
  { name: 'Sisterhood', pos: 15, notice: ORGANIZA_NOTE, image: '/img/areas/sisterhood.jpg', icon: '🌸', desc: 'Comunidad de mujeres de todas las generaciones.', teams: [
  ...
  { name: 'Jóvenes', pos: 10, notice: ORGANIZA_NOTE, image: '/img/areas/jovenes.jpg', icon: '⚡', desc: 'Comunidades para adolescentes y jóvenes, incluyendo Grupos de Conexión y eventos.', teams: [
    ['Youth', 'Comunidad para adolescentes de 12 a 18 años.'],
    ['PowerHouse', 'Comunidad para jóvenes de 18 a 30 años.'] ] },
  { name: 'Cuidado Pastoral', pos: 10, image: '/img/areas/cuidado-pastoral.jpg', icon: '💛', desc: 'Acompañamiento espiritual en diferentes momentos de la vida.', teams: [
  ...
  { name: 'CityCare', pos: 10, notice: ORGANIZA_NOTE, image: '/img/areas/citycare.jpg', icon: '🏙️', desc: 'Engloba la acción social de nuestra iglesia.', teams: [
  ...
  { name: 'Formaciones', pos: 25, notice: ORGANIZA_NOTE, image: '/img/areas/formaciones.jpg', icon: '📖', desc: 'Espacios para crecer en Biblia, fe y liderazgo.', teams: [
  ...
  { name: 'IT', pos: 40, image: '/img/areas/it.jpg', icon: '💻', desc: 'Tecnología y herramientas digitales para la vida de la iglesia.', teams: [
...
function seed({ cities = false } = {}) {
  ...
  const insArea = db.prepare('INSERT INTO teams (name, description, icon, image_url, notice, image_pos, sort) VALUES (?,?,?,?,?,?,?)');
  ...
  AREAS.forEach((a, i) => {
    ...
    if (!area) { area = { id: Number(insArea.run(a.name, a.desc, a.icon, a.image || '', a.notice || '', a.pos ?? 30, i + 1).lastInsertRowid) }; created++; }
    else {
      ...
      // El enfoque de la foto solo se ajusta si sigue en el valor por defecto (30): no pisa lo que se haya elegido en el panel
      if (a.pos !== undefined && a.pos !== 30) db.prepare('UPDATE teams SET image_pos = ? WHERE id = ? AND image_pos = 30').run(a.pos, area.id);
    }
